...
        m_currentRotationAngle = m_currentRotationAngle + CharacterInput.Instance.CamerVector.x * m_smooth*Time.deltaTime*20f;
        //更改当前高度
        m_currentHeight = m_transsform.eulerAngles.x + CharacterInput.Instance.CamerVector.y * m_smooth * Time.deltaTime*10f;
        if (m_currentHeight >50)
        {
            m_currentHeight = 50;
            if (m_distanceAway < 8f)  m_distanceAway += Time.deltaTime;
        }
        if (m_currentHeight < 10)
        {
            m_currentHeight = 10;
            if (m_distanceAway > 4f)  m_distanceAway -= Time.deltaTime;
        }
        //返回一个Y轴旋转玩家当前角度那么多的度数
        Quaternion m_currentRotation = Quaternion.Euler(m_currentHeight, m_currentRotationAngle, 0);
        //玩家的位置
...
        m_transsform.LookAt(m_player);
    ...
    {
        //实现滑轮拖动
        if(m_distanceAway>4f) m_distanceAway -= Input.GetAxis("Mouse ScrollWheel");
    }
}
